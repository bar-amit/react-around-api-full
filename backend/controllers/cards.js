...
async function deleteCard(req, res) {
  ...
  try {
    ...
    if (!cardToDelete) {
      const CardNotFoundError = new Error('User ID not found');
    ...
    if (err.statusCode === 404 || err.statusCode === 403 || err.statusCode === 400) {
...
async function likeCard(req, res) {
  ...
  try {
    ...
    const updatedCard = req.method === 'PUT'
      ? await card.findByIdAndUpdate(cardId, { $addToSet: { likes: userId } }, { new: true })
      : await card.findByIdAndUpdate(cardId, { $pull: { likes: userId } }, { new: true });
    if (!updatedCard) {
